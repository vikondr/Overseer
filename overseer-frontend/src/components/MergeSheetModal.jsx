...
import { previewMerge, commitMerge } from '../api/sheets';
...
export default function MergeSheetModal({
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    previewMerge(projectId, forkSheet.id)
      .then((data) => {
        if (cancelled) return;
        setPreview(data);
        const initial = {};
  ...
  const toggleDiff = async (file) => {
    ...
    try {
      const [a, b] = await Promise.all([
        fetchFileBlob(file.parentFile.id),
        fetchFileBlob(file.forkFile.id),
      ]);
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const result = await commitMerge(projectId, forkSheet.id, payload);
      onDone(result);
    } catch (e) {
      setError(e.message);
    } finally {
